...
object StatusIndex {
    val table = new Hashtable[String, StatusIndex]();

    def open(file: String): StatusIndex = {
        table.synchronized {
...
class StatusIndex(_file: String) extends IClose {
    ...
    override def close() {
        StatusIndex.table.synchronized {
            if (this.refrence == 0) {
                StatusIndex.table.remove(this.file);
