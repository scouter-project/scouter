...
public class Logger {
	...
	static {
		...
		Thread t = new Thread(proc,"scouter.server.Logger");
		t.setDaemon(true);
		t.start();
	}
	...
	private static synchronized void openFile() throws IOException {
		if (pw == null) {
			...
			if (conf.log_rotation) {
				FileWriter fw = new FileWriter(new File(conf.logs_dir, "server-" + DateUtil.yyyymmdd()+".log"), true);
	...
	protected static void clearOldLog() {
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (x < 0)
				continue;
			String date = name.substring("server-".length(),x);
			if (date.length() != 8)
				continue;
	...
	public static void main(String[] args) {
		String name = "server-19701123.log";
		int x = name.lastIndexOf('.');
		String date = name.substring("server-".length(),x);
