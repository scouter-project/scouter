...
public class P0SetMV extends LocalVariablesSorter implements Opcodes {
	...
	public P0SetMV(int access, String name, String desc, MethodVisitor mv, String owner) {
	...
	public void visitCode() {
		...
		if (name.equals("setNull")) {
			...
			mv.visitMethodInsn(Opcodes.INVOKESTATIC, TRACESQL, "set",
					"(Lscouter/agent/trace/SqlParameter;ILjava/lang/String;)V");
		} else {
			Type tp = args[1];
			switch (tp.getSort()) {
			case Type.BOOLEAN:
			...
			case Type.OBJECT:
				...
				if (tp.equals(AsmUtil.stringType)) {
					...
					mv.visitMethodInsn(Opcodes.INVOKESTATIC, TRACESQL, "set",
							"(Lscouter/agent/trace/SqlParameter;ILjava/lang/Object;)V");
				}
				break;
			default:
				mv.visitVarInsn(Opcodes.ALOAD, 2);
