...
public class TraceApiCall {
	...
	public static void endApicall(Object stat, Object returnValue, Throwable thr) {
		...
		try {
			...
			statistics.process(tctx.serviceHash, step);
		} catch (Throwable t) {
			t.printStackTrace();
		}
	}
	...
	public static Object startSocket(Socket socket, SocketAddress addr, int timeout) {
			...
			if (conf.debug_socket_openstack) {
				try {
					...
					if (conf.debug_socket_openstack_port == 0 || conf.debug_socket_openstack_port == inet.getPort()) {
						Logger.println("SOCKET " + inet.getHostName() + ":" + inet.getPort() + " not profiled!!");
						Logger.println(ThreadUtil.getStackTrace(Thread.currentThread().getStackTrace()));
					}
				} catch (Throwable t) {
				}
			}
		...
		try {
			...
			Logger.println("TA018", "socket trace error", t);
			return null;
		}
	}
		...
		try {
			...
			if (conf.debug_socket_openstack) {
				if (conf.debug_socket_openstack_port == 0 || conf.debug_socket_openstack_port == step.port) {
					Logger.println("SOCKET " + IPUtil.toString(step.ipaddr) + ":" + step.port);
					Logger.println(ThreadUtil.getStackTrace(Thread.currentThread().getStackTrace()));
				}
			}
		} catch (Throwable t) {
			Logger.println("TA016", "socket trace close error", t);
		}
	}
