...
public class HttpClient5Factory {
	...
	private static final String HTTP_CLIENT5 = "scouter.xtra.httpclient.HttpClient5";
	...
	public static IHttpClient create(ClassLoader parent) {
		try {
			...
			Class c = Class.forName(HTTP_CLIENT5, true, loader);
			return (IHttpClient) c.newInstance();
		} catch (Throwable e) {
			e.printStackTrace();
			Logger.println("A132-5", "fail to create", e);
			return dummy;
		}
	}
}
