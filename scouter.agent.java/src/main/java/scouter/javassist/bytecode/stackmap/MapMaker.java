...
public class MapMaker extends Tracer {
    ...
    */
    ...
    private void merge(TypeData td, TypeData target) {
        ...
        if (td != TypeTag.TOP && td.isObjectType())
            tdIsObj = true;

        if (target != TypeTag.TOP && target.isObjectType())
            targetIsObj = true;
    ...
    private void toStackMapBody(StackMapTable.Writer writer, TypedBlock bb,
        ...
        if (stackTop == 0) {
            ...
            TypeData td = bb.stackTypes[0];
            if (td != TypeTag.TOP && td.is2WordType()) {
    ...
    private static boolean stackMapEq(TypeData[] oldTd, TypeData[] newTd, int len) {
        for (int i = 0; i < len; i++) {
            TypeData td = oldTd[i];
            if (td == TypeTag.TOP) {        // the next element to LONG/DOUBLE is TOP.
                if (newTd[i] != TypeTag.TOP)
                    return false;
            }
            else
