...
import scouter.xtra.http.UseridUtil;
...
public class JakartaHttpTraceBase implements IHttpTrace {
    ...
    public JakartaHttpTraceBase() {
        ...
        ConfObserver.add(JakartaHttpTraceBase.class.getName(), new Runnable() {
            public void run() {
                String x = Configure.getInstance().trace_http_client_ip_header_key;

                if (!CompareUtil.equals(x, http_remote_ip_header_key)) {
                    remote_by_header = !StringUtil.isEmpty(x);
