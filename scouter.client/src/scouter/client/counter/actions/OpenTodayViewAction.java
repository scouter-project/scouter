	...
	public void run() {
		if (window != null) {
			try {
				...
				CounterRealDateView view = (CounterRealDateView) win.getActivePage().showView(
						CounterRealDateView.ID, Hexa32.toString32(objHash) + "-" + objType+counter+serverId,
						IWorkbenchPage.VIEW_ACTIVATE);
				if (view != null) {
					try {
						view.setInput(objHash, objName, objType, counter, serverId);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
